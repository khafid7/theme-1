...
<body class="bg-texture text-charcoal font-sans antialiased overflow-x-hidden no-scroll" id="mainBody">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
                <div class="group relative flex flex-col items-center">
                    <div class="relative w-full max-w-sm aspect-[3/4] mb-6">
                        ...
                        <img src="{{ $invitation->groom_photo }}" class="relative w-full h-full object-cover rounded-t-[10rem] rounded-b-[2rem] border-4 border-white shadow-xl transform -rotate-3 z-10">
                    </div>
                    <div class="relative z-20 text-center">
                        <h3 class="font-rustic-script text-5xl text-charcoal mb-1">{{ $invitation->groom_name }}</h3>
                        <p class="text-xs font-bold text-sage uppercase tracking-widest mb-3">Putra Bpk. {{ $invitation->groom_father }}</p>
                        @if($invitation->groom_instagram)
                        <a href="{{ $invitation->groom_instagram }}" target="_blank" class="inline-flex items-center gap-1 text-xs bg-white border border-sage px-3 py-1 rounded-full hover:bg-sage hover:text-white transition cursor-pointer z-30 relative text-sage-dark">
                ...
                <div class="group relative flex flex-col items-center">
                    <div class="relative w-full max-w-sm aspect-[3/4] mb-6">
                        ...
                        <img src="{{ $invitation->bride_photo }}" class="relative w-full h-full object-cover rounded-t-[10rem] rounded-b-[2rem] border-4 border-white shadow-xl transform rotate-3 z-10">
                    </div>
                    <div class="relative z-20 text-center">
                        <h3 class="font-rustic-script text-5xl text-charcoal mb-1">{{ $invitation->bride_name }}</h3>
                        <p class="text-xs font-bold text-sage uppercase tracking-widest mb-3">Putri Bpk. {{ $invitation->bride_father }}</p>
                        @if($invitation->bride_instagram)
                        <a href="{{ $invitation->bride_instagram }}" target="_blank" class="inline-flex items-center gap-1 text-xs bg-white border border-sage px-3 py-1 rounded-full hover:bg-sage hover:text-white transition cursor-pointer z-30 relative text-sage-dark">
                            <i class="ph-fill ph-instagram-logo"></i> <span>Instagram</span>
                        </a>
                        @endif
                    </div>
    ...
    <section class="relative bg-sage-dark text-cream pt-12 pb-12 px-6">
        <div class="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
            <div class="bg-white/5 p-8 border border-white/10 rounded-tr-[3rem] rounded-bl-[3rem] text-center transform hover:-translate-y-2 transition duration-300">
                ...
                <h3 class="font-serif text-2xl mb-2">{{ $invitation->akad_title }}</h3>
                <p class="text-3xl font-bold my-4 text-white">{{ $invitation->akad_datetime->format('H:i') }} WIB</p>
                <p class="opacity-80 mb-6">{{ $invitation->akad_location }}</p>
                <a href="{{ $invitation->akad_map_link }}" class="inline-block border border-cream px-6 py-2 text-xs uppercase hover:bg-cream hover:text-sage-dark transition rounded-full">Google Maps</a>
            </div>
            ...
            <div class="bg-white/5 p-8 border border-white/10 rounded-tl-[3rem] rounded-br-[3rem] text-center transform hover:-translate-y-2 transition duration-300">
                ...
                <h3 class="font-serif text-2xl mb-2">{{ $invitation->resepsi_title }}</h3>
                <p class="text-3xl font-bold my-4 text-white">{{ $invitation->resepsi_datetime->format('H:i') }} WIB</p>
                <p class="opacity-80 mb-6">{{ $invitation->resepsi_location }}</p>
                <a href="{{ $invitation->resepsi_map_link }}" class="inline-block border border-cream px-6 py-2 text-xs uppercase hover:bg-cream hover:text-sage-dark transition rounded-full">Google Maps</a>
            </div>
        </div>
    </section>
    ...
    <section class="relative bg-sage-dark text-cream pt-12 pb-12 px-6">
        <div class="max-w-2xl mx-auto text-center relative z-10">
            ...
            <div class="bg-white/10 backdrop-blur-sm p-8 rounded-xl shadow-lg border border-white/20 relative overflow-hidden">
                ...
                <p class="text-sm uppercase tracking-widest opacity-70 mb-2">{{ $invitation->bank_name }}</p>
                <h3 class="font-mono text-3xl mb-2 tracking-wider" id="noRek">{{ $invitation->bank_number }}</h3>
                <p class="text-sm font-bold mb-6">a.n {{ $invitation->bank_holder }}</p>
            ...
            <div class="mt-8 bg-white/10 backdrop-blur-sm p-6 rounded-xl shadow border border-white/20">
                ...
                <p class="text-sm opacity-80 mb-4">{{ $invitation->gift_address }}</p>
            </div>
        </div>
    </section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div>
                    <form action="{{ route('kirim.ucapan') }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            <select name="kehadiran" class="w-full bg-white border-b-2 border-sage/30 p-3 text-sage-dark focus:outline-none focus:border-terracotta transition bg-transparent cursor-pointer">
                        ...
                        <button type="submit" class="w-full bg-sage-dark text-white font-bold py-3 rounded-full hover:bg-terracotta hover:shadow-lg transform hover:-translate-y-1 transition duration-300 tracking-widest text-sm">
                            KIRIM UCAPAN
                        </button>
                    </form>
                </div>
                    ...
                    <div class="rustic-scroll h-[400px] overflow-y-auto pr-2">
                        @if($invitation->comments->count() > 0)
                            @foreach($invitation->comments as $comment)
                                <div class="rustic-comment-card group">
                                    <div class="flex justify-between items-start mb-2">
                                        <div>
                                            <h4 class="font-bold text-sage-dark font-serif text-lg">{{ $comment->nama }}</h4>
                                        ...
                                        @elseif($comment->kehadiran == 'Tidak Hadir')
    ...
    <script>
        function bukaUndangan() { 
            ...
            setTimeout(() => { musicBtn.classList.remove('hidden'); }, 1000); 
            toggleMusic(); 
        }
        ...
        const timer = setInterval(function() { 
            ...
            const boxClass = "bg-white/10 backdrop-blur-md border border-white/30 px-4 py-2 rounded text-center min-w-[70px]"; 
            const numClass = "text-2xl font-bold block"; 
            const labelClass = "text-[10px] uppercase tracking-widest opacity-80"; 
            document.getElementById("countdown").innerHTML = `<div class="${boxClass}"><span class="${numClass}">${days}</span><span class="${labelClass}">Hari</span></div><div class="${boxClass}"><span class="${numClass}">${hours}</span><span class="${labelClass}">Jam</span></div><div class="${boxClass}"><span class="${numClass}">${minutes}</span><span class="${labelClass}">Menit</span></div><div class="${boxClass}"><span class="${numClass}">${seconds}</span><span class="${labelClass}">Detik</span></div>`; 
        }, 1000);
